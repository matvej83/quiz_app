...
class QuizCubit extends Cubit<QuizState> {
  ...
  void loadWords() async {
    try {
      ...
      if (words.isEmpty) {
        emit(QuizCompleted(correctAnswers: 0, totalQuestions: 0));
      } else {
        emit(QuizLoaded(
          words: words,
          currentIndex: 0,
  ...
  void checkAnswer(String userAnswer) {
    ...
    final isCorrect =
        userAnswer.trim().toLowerCase() == currentWord.russianWord.toLowerCase();
    emit(QuizLoaded(
  ...
  void nextQuestion() {
    ...
    if (nextIndex >= loadedState.words.length) {
      ...
      final finalCorrect = loadedState.correctCount +
          (loadedState.correct ? 1 : 0);
      emit(QuizCompleted(
